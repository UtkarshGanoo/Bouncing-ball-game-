...
const collisionSound = new Audio('collision2.mp3');
const scoreSound = new Audio('score.mp3');
const lifeLostSound = new Audio('lifeLost2.mp3');
const backgroundMusic = new Audio('background.mp3'); // Background music
...
function drawBall() {
  ...
  ctx.fillStyle = "white";
  ctx.beginPath();
  ctx.arc(ball.x - 7, ball.y - 5, 5, 0, Math.PI * 2);
  ctx.arc(ball.x + 7, ball.y - 5, 5, 0, Math.PI * 2);
  ctx.fill();

  ctx.fillStyle = "black";
  ctx.beginPath();
  ctx.arc(ball.x - 7 + Math.random() * 2 - 1, ball.y - 5 + Math.random() * 2 - 1, 2, 0, Math.PI * 2);
  ctx.arc(ball.x + 7 + Math.random() * 2 - 1, ball.y - 5 + Math.random() * 2 - 1, 2, 0, Math.PI * 2);
  ctx.fill();
}
...
function updateBackgroundStyle() {
  switch (currentBackground) {
    case "daytime":
      canvas.style.background = "#81BFDA";
      break;
    case "nighttime":
      canvas.style.background = "#074799";
      break;
    case "garden":
      canvas.style.background = "#D6CFB4";
      break;
    case "snow":
      canvas.style.background = "#E5D9F2";
      break;
  }
}
